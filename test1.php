...
while (true)
{
    ...
    if ($time + $tickStep <= microtime(true)) {
        ...
        while (count($connections) < $maxConnections) {
            ...
            $connections[] = createConnection($host, $user, $pass, $db);
...
function createConnection($host, $user, $pass, $db)
{
    ...
    $link  = mysql_connect($host, $user, $pass, false);
...
function fetchData($connection, $dataMultiplier, $query)
{
    ...
    if (!$resource) {
        ...
        var_dump($connection);
        $resourceId = trim(ob_get_clean());
        echo sprintf("[%s][%f0] got data %s  length: %s ".PHP_EOL, date('H:i:s'), $diff, $resourceId,  strlen(serialize($data)));
    }
}
